...
class FBFG_Post_Assets {
	 ...
	 * @var fbfg_flag
	 */
	public $fbfg_flag = false;

	/**
	 * FBFG FAQ Layout Flag
	 *
	 * @since 1.0.0
	 * @var fbfg_faq_layout
	 */
	public $fbfg_faq_layout = false;

	/**
	 * FBFG File Generation Flag
	 *
	 * @since 1.0.0
	 * @var file_generation
	 */
	...
	/**
	 * FBFG File Generation Fallback Flag for CSS
	 *
	 * @since 1.0.0
	 * @var file_generation
	 */
	...
	/**
	 * FBFG File Generation Fallback Flag for JS
	 *
	 * @since 1.0.0
	 * @var file_generation
	 */
	...
	public function allow_assets_generation() {
		...
		$page_assets     = get_post_meta( $this->post_id, '_fbfg_page_assets', true );
		$version_updated = false;
		$css_asset_info  = array();
		$js_asset_info   = array();

		if ( empty( $page_assets ) || empty( $page_assets['fbfg_version'] ) ) {
			return true;
		}

		if ( FBFG_ASSET_VER !== $page_assets['fbfg_version'] ) {
			$version_updated = true;
		}

		if ( 'enabled' === $this->file_generation ) {

			$css_file_name = get_post_meta( $this->post_id, '_fbfg_css_file_name', true );
			$js_file_name  = get_post_meta( $this->post_id, '_fbfg_js_file_name', true );
			...
			if ( $version_updated ) {
				$fbfg_filesystem = fbfg_filesystem();

				if ( ! empty( $css_file_path ) ) {
					$fbfg_filesystem->delete( $css_file_path );
				}

				if ( ! empty( $js_file_path ) ) {
					$fbfg_filesystem->delete( $js_file_path );
				}

				// Delete keys.
				delete_post_meta( $this->post_id, '_fbfg_css_file_name' );
				delete_post_meta( $this->post_id, '_fbfg_js_file_name' );
			}
	...
	public function update_page_assets() {
		...
		$meta_array = array(
			...
			'fbfg_flag'           => $this->fbfg_flag,
			'fbfg_version'        => FBFG_ASSET_VER,
			'gfonts'             => $this->gfonts,
			'fbfg_faq_layout'     => $this->fbfg_faq_layout,
		);

		update_post_meta( $this->post_id, '_fbfg_page_assets', $meta_array );
	}
	/**
	 * This is the action where we create dynamic asset files.
	 * CSS Path : uploads/fbfg-plugin/fbfg-style-{post_id}-{timestamp}.css
	 * JS Path : uploads/fbfg-plugin/fbfg-script-{post_id}-{timestamp}.js
	 *
	 * @since 1.0.0
	 */
	...
	public function enqueue_blocks_dependency_frontend() {
		...
		foreach ( $block_list_for_assets as $key => $curr_block_name ) {
			...
			foreach ( $js_assets as $asset_handle => $val ) {
				// Scripts.
				if ( 'fbfg-faq-js' === $val ) {
					if ( $this->fbfg_faq_layout ) {
						wp_enqueue_script( 'fbfg-faq-js' );
					}
				} else {
	...
	public function enqueue_file_generation_assets() {
		...
		if ( isset( $file_handler['css_url'] ) ) {
			wp_enqueue_style( 'fbfg-style-' . $this->post_id, $file_handler['css_url'], array(), FBFG_VER, 'all' );
		} else {
			$this->fallback_css = true;
		}
		if ( isset( $file_handler['js_url'] ) ) {
			wp_enqueue_script( 'fbfg-script-' . $this->post_id, $file_handler['js_url'], array(), FBFG_VER, true );
		} else {
	...
	public function print_script() {
		...
		echo '<script type="text/javascript" id="fbfg-script-frontend-' . $this->post_id . '">' . $this->script . '</script>'; //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
	}
	...
	public function print_stylesheet() {
		...
		echo '<style id="fbfg-style-frontend-' . $this->post_id . '">' . $this->stylesheet . '</style>'; //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
	}
	...
	public function print_conditional_css() {
		...
		if ( in_array( 'fbfg/masonry-gallery', $this->current_block_list, true ) ) {
			$conditional_block_css .= FBFG_Block_Helper::get_masonry_gallery_css();
		}

		echo '<style id="fbfg-style-conditional-extension">' . $conditional_block_css . '</style>'; //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
	...
	public function print_google_fonts() {
		...
		$show_google_fonts = apply_filters( 'fbfg_blocks_show_google_fonts', true );
	...
	public function get_block_css_and_js( $block ) {
		...
		if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
			 ...
			 */
			$blockattr = apply_filters( 'fbfg_block_attributes_for_css_and_js', $block['attrs'], $name );
		...
		if ( 'core/gallery' === $name && isset( $block['attrs']['masonry'] ) && true === $block['attrs']['masonry'] ) {
			$this->current_block_list[] = 'fbfg/masonry-gallery';
			$this->fbfg_flag             = true;
			$css                       += FBFG_Block_Helper::get_gallery_css( $blockattr, $block_id );
		}

		if ( strpos( $name, 'fbfg/' ) !== false ) {
			$this->fbfg_flag = true;
		}
	...
	public function create_file( $file_data, $type, $file_state = 'new', $old_file_name = '' ) {
		...
		$file_system   = fbfg_filesystem();

		// Example 'fbfg-css-15-1645698679.css'.
		$file_name = 'fbfg-' . $type . '-' . $this->post_id . '-' . $new_timestamp . '.' . $type;
		...
		if ( $result ) {
			...
			update_post_meta( $this->post_id, '_fbfg_' . $type . '_file_name', $file_name );
		}
	...
	public function file_write( $file_data, $type = 'css', $post_id = '' ) {
		...
		$file_name   = get_post_meta( $this->post_id, '_fbfg_' . $type . '_file_name', true );
		...
		if ( '' === $file_data ) {
			 ...
			 * This means this page does not use FBFG block.
