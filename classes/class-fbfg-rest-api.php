...
if ( ! class_exists( 'FBFG_Rest_API' ) ) {

	/**
	 * Class FBFG_Rest_API.
	 */
	final class FBFG_Rest_API {
		...
		public function delete_page_assets( $post_id ) {
			...
			if ( 'enabled' === FBFG_Helper::$file_generation ) {

				$css_asset_info = FBFG_Scripts_Utils::get_asset_info( 'css', $post_id );
				$js_asset_info  = FBFG_Scripts_Utils::get_asset_info( 'js', $post_id );
			...
			delete_post_meta( $post_id, '_fbfg_page_assets' );
			delete_post_meta( $post_id, '_fbfg_css_file_name' );
			delete_post_meta( $post_id, '_fbfg_js_file_name' );
		...
		public function blocks_register_rest_fields() {
			$post_type = FBFG_Helper::get_post_types();

			foreach ( $post_type as $key => $value ) {
				// Add featured image source.
				register_rest_field(
					$value['value'],
					'fbfg_featured_image_src',
					array(
	...
	/**
	 *  Prepare if class 'FBFG_Rest_API' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	FBFG_Rest_API::get_instance();
}
