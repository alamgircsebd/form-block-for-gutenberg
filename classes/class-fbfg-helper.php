...
if ( ! class_exists( 'FBFG_Helper' ) ) {
	...
	final class FBFG_Helper {
		 ...
		 * @var fbfg_faq_layout
		 */
		public static $fbfg_faq_layout = false;
		...
		public function __construct() {
			require FBFG_DIR . 'classes/class-fbfg-config.php';
			require FBFG_DIR . 'classes/class-fbfg-block-helper.php';
			require FBFG_DIR . 'classes/class-fbfg-block-js.php';
		...
		public static function backend_load_font_awesome_icons() {
			...
			$json_file = FBFG_DIR . 'blocks-config/fbfg-controls/FBFGIcon.json';
			...
			$str             = fbfg_filesystem()->get_contents( $json_file );
		...
		public static function get_query( $attributes, $block_type ) {
			...
			$query_args = apply_filters( "fbfg_post_query_args_{$block_type}", $query_args, $attributes );
		...
		public static function get_image_sizes() {
			...
			$image_sizes[] = array(
				'value' => 'full',
				'label' => esc_html__( 'Full', 'form-block-for-gutenberg' ),
			);
			...
			$image_sizes = apply_filters( 'fbfg_post_featured_image_sizes', $image_sizes );
		...
		public static function get_post_types() {
			...
			return apply_filters( 'fbfg_loop_post_types', $options );
		}
		...
		public static function get_related_taxonomy() {
			...
			return apply_filters( 'fbfg_post_loop_taxonomies', $return_array );
		}
		...
		public static function get_taxonomy_list() {
			...
			return apply_filters( 'fbfg_taxonomies_list', $return_array );
		}
		...
		/**
		 * Get FBFG upload dir path.
		 *
		 * @since 1.0.0
		 * @return string
		 */
		public static function get_fbfg_upload_dir_path() {
		...
		/**
		 * Get FBFG upload url path.
		 *
		 * @since 1.0.0
		 * @return string
		 */
		public static function get_fbfg_upload_url_path() {
			$wp_info = self::get_upload_dir();
		...
		/**
		 * Delete all files from FBFG upload dir.
		 *
		 * @since 1.0.0
		 * @return string
		 */
		public static function delete_all_fbfg_dir_files() {
			$dir           = self::get_fbfg_upload_dir_path();
			$wp_filesystem = fbfg_filesystem();
		...
		public static function allow_file_generation() {
			return get_option( '_fbfg_allow_file_generation', 'disabled' );
		}

		/**
		 * Check if FBFG upload folder has write permissions or not.
		 *
		 * @since 1.0.0
		 * @return bool true or false.
		 */
		public static function is_fbfg_dir_has_write_permissions() {
			$upload_dir = self::get_upload_dir();

			return fbfg_filesystem()->is_writable( $upload_dir['path'] );
		}
		/**
		...
		public function get_post_assets_instance() {
			return fbfg_get_front_post_assets();
		}
		...
		public function generate_assets() {
			...
			if ( is_single() || is_page() || is_404() ) {
				 ...
				 */
				$this_post = apply_filters( 'fbfg_post_for_stylesheet', $this_post );
